...
flowchart TD
    ...
    subgraph "Phase 2: Intelligent Preprocessing (LangGraph)"
        ...
        G --> H[Cleaned JSON / Markdown / HTML 재생성]
    end

    subgraph "Phase 3: Output Boundary"
        H --> I[Chunking / Vector DB 적재 준비]
    end
...
| H | 정제된 element를 기준으로 `cleaned.json`, `cleaned.md`, `preview.html`을 재생성하는 단계 | 구현 완료 |
| I | 이후 chunking, embedding, vector DB 적재로 연결되는 출력 경계 | 예정 |
