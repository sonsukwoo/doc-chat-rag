...
현재 stage5는 아직 production 마감 상태는 아니지만, **room-aware retrieval + ToolNode + grounded compose answer**까지는 연결된 상태입니다.

- Postgres
  - `app_chat`, `app_doc`, `app_pipeline`, `app_checkpoint` schema를 사용합니다.
  - LangGraph `PostgresSaver` checkpoint는 `app_checkpoint`에 저장됩니다.
- stage3 indexing
  - `room_id`를 받으면 Qdrant payload와 point id에 반영합니다.
  - 같은 공용 컬렉션에서도 `room_id + document_id` 범위로 재업로드/삭제할 수 있게 했습니다.
- stage4 retrieval
  - `room_id + active_document_ids` filter를 받는 room-aware 검색 경로를 지원합니다.
  - artifact 기반 retrieval pipeline과 별도로, stage5에서 바로 호출할 수 있는 `search_room_knowledge()` 서비스 함수를 둡니다.
- stage5 chatbot
  - `room_id`로 Postgres room context를 읽고, 해당 room의 active document 목록을 기본 검색 범위로 사용합니다.
  - `search_room_knowledge` 툴은 실제 stage4 room-scoped 검색을 호출합니다.
  - 툴은 모델 인자로 `room_id`를 직접 받지 않고, LangGraph `runtime.state`에서 room/document 범위를 읽습니다.
  - `agent_llm -> ToolNode -> grounding_check -> fallback_or_retrieve_deeper -> compose_answer_with_citations` 흐름이 실제로 연결되어 있습니다.
  - 현재는 `expand_context_window`, `load_visual_asset`, `web_search`가 scaffold 상태이며, retrieval 이후 deeper expansion과 visual lookup 고도화는 다음 단계 범위입니다.
