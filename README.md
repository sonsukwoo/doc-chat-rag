...
- 최종적으로 `cleaned.json`, `cleaned.md`, `preview.html`, `chunks.json`, `chunks.jsonl`, `chunks.md`, `indexing.json`까지 재생성하기
...
- 구현 완료: `Docling 기반 1차 파싱`, `LangGraph 기반 2차 전처리`, `figure/table crop`, `figure fan-out review`, `figure keep/drop 판정`, `table summary prepare-route-text/vlm 분기`, `문서 프로파일 추론`, `caption 연결/중복 정리`, `visual bbox 순서 보정`, `Markdown/HTML 재생성`, `stage3 구조 기반 chunking`, `semantic split 보조`, `chunk preview Markdown 생성`, `dense embedding 생성`, `Qdrant upsert`
- 보수적으로 유지: `본문 텍스트는 가능한 한 원형 보존`
- 아직 분리 예정: `OCR Docker 레이어`, `image PDF 자동 라우팅`, `stage4 검색 API`, `hybrid retrieval`, `chat serving layer`

즉, 현재 저장소는 **입력 PDF를 바로 Docling에 넣을 수 있다는 전제 아래**, `stage1(raw extraction) -> stage2(intelligent preprocessing) -> stage3(chunking + dense indexing)` 흐름을 구현하고 있습니다. OCR 라우팅과 검색 API는 아직 메인 실행 플로우에 포함되어 있지 않으며, search/top-k 반환은 이후 별도 stage로 분리할 예정입니다.
...
flowchart TD
    ...
    subgraph "Phase 3: Chunking + Indexing"
        ...
        J --> K[Semantic Split / Merge 보조\n: Ollama bge-m3 optional]
        K --> L[chunks.json / chunks.jsonl / chunks.md 생성]
        L --> M[Dense Embedding 생성 및 Qdrant Upsert\n: same bge-m3 vector space]
    end

    subgraph "Phase 4: Retrieval Boundary"
        M --> N[Stage 4 Search / Top-k Retrieval 예정]
    end
...
| K | 긴 prose chunk는 semantic split, 지나치게 작은 prose chunk는 semantic merge를 보조적으로 시도하는 단계 | 구현 완료 (`bge-m3`, optional) |
| L | `chunks.json`, `chunks.jsonl`, `chunks.md`를 저장하는 단계 | 구현 완료 |
| M | 같은 embedding 모델로 dense vector를 만들고 Qdrant에 upsert 하는 단계 | 구현 완료 (`backend/stage3_indexing/`) |
| N | 질의 embedding, cosine/hybrid 검색, top-k 반환을 담당하는 검색 경계 | 예정 |

---

## 🔍 현재 코드 기준 실제 처리 흐름

현재 저장소에서 실제로 돌아가는 흐름은 아래 세 단계입니다.
...
4. 긴 `prose` chunk는 필요할 때 semantic split 시도
5. 짧은 `prose` chunk는 필요할 때 semantic merge 시도
6. 최종적으로 `chunks.json`, `chunks.jsonl`, `chunks.md` 저장
7. 같은 embedding 모델로 dense vector를 만들고 Qdrant에 upsert
...
현재 전략의 핵심은 **structure-first chunking**입니다. 이 프로젝트는 이미 stage1에서 Docling이 문서를 `heading / paragraph / table / figure / caption` 단위로 분해해 주기 때문에, 일반적인 문자열 splitter로 다시 문서를 잘게 자르는 것보다 **문서 구조를 먼저 보존한 뒤 필요한 부분에만 semantic 보정을 붙이는 방식**이 더 안정적입니다.
...
- semantic split/merge는 모든 chunk에 강제하지 않고, 긴/짧은 `prose`에서만 보조적으로 써서 과도한 자동 분해를 막습니다.

즉 현재 철학은 **규칙 기반 구조 청킹이 메인이고, semantic은 예외 보정**입니다.
...
- `chunks.json` / `chunks.jsonl`
  ...
  - `heading_path`, `overlap_text`, `caption`, `summary_text`는 `metadata`에 별도 저장
- `chunks.md`
...
#### dense indexing은 어떻게 붙는가?

현재는 semantic split/merge에 쓰는 것과 같은 OpenAI 호환 embedding client를 재사용해, chunk 생성 직후 dense embedding을 만들고 Qdrant에 upsert 합니다. 이렇게 둔 이유는 다음과 같습니다.

- chunk를 나누는 의미 공간과 검색 벡터 공간을 초기에는 동일하게 맞춰 변수 수를 줄이기 위함
- `python -m backend.stage3` 한 번으로 chunk 생성과 업로드까지 끝내기 위함
- 이후 search/hybrid 실험은 stage4에서 분리해 진행하기 위함
...
#### semantic split / merge는 어떻게 붙는가?

semantic 단계는 항상 강제되지 않고 **선택적 보조 단계**로 동작합니다.

- embedding client는 OpenAI 호환 `embeddings.create(...)` 인터페이스를 사용
- 기본값은 로컬 Ollama의 `bge-m3`
- semantic 호출이 실패하면 전체 pipeline을 멈추지 않고 구조 청킹 결과로 fallback

현재 기준:

- `split`: 너무 긴 `prose` chunk에서만 시도
- `merge`: 같은 `heading_path` 안의 인접한 짧은 `prose` chunk에서만 시도
- `table`, `figure`에는 semantic split/merge를 적용하지 않음

즉 stage3의 기본 철학은 **구조 기반 청킹이 메인, semantic은 예외 보정**입니다.
...
├── stage3_chunking/
│   ├── config.py              # chunking / semantic runtime 설정
│   ├── embeddings.py          # OpenAI 호환 embedding helper
│   ├── pipeline.py            # 구조 청킹 + semantic split/merge
...
- `chunks.json`: stage3 최종 chunk payload 목록
- `chunks.jsonl`: vector DB 적재나 후속 배치 처리에 바로 쓰기 좋은 line-delimited chunk 출력
- `chunks.md`: 사람이 chunk 품질을 검수하기 위한 preview Markdown
- `indexing.json`: stage3 dense indexing 결과 manifest
...
여기서 중요한 점은, `heading_path`와 `overlap_text`는 metadata로는 남기되, 실제 `text` 안에는 `섹션:` / `이전 문맥:` 같은 라벨을 넣지 않는다는 것입니다. 즉 `chunks.json`은 embedding 친화적으로 유지하고, 사람이 읽기 좋은 표현은 `chunks.md`에서만 보여줍니다.
...
STAGE3_QDRANT_COLLECTION_NAME=rag_chat
...
- `STAGE3_ENABLE_SEMANTIC`: stage3 semantic split/merge 사용 여부
- `STAGE3_EMBEDDING_BASE_URL`: OpenAI 호환 embeddings 엔드포인트
- `STAGE3_EMBEDDING_MODEL`: semantic split/merge와 dense indexing에 공통으로 사용할 모델명
- `STAGE3_ENABLE_INDEXING`: chunk 생성 후 Qdrant 업로드까지 이어서 수행할지 여부
- `STAGE3_QDRANT_URL`: stage3 dense indexing 대상 Qdrant 주소
...
실행 후 `chunks.json`, `chunks.jsonl`, `chunks.md`, `indexing.json`이 생성됩니다.

- semantic embeddings가 정상 연결되면 `semantic_split` / `semantic_merge`가 적용될 수 있습니다.
- 연결 실패 시에는 구조 청킹 결과만 저장하고, 상태를 `completed_with_semantic_fallback`로 남깁니다.
- indexing이 켜져 있고 Qdrant 설정이 있으면, 같은 embedding 모델로 dense vector를 생성해 Qdrant에 upsert 합니다.
- search/top-k 반환은 아직 별도 stage로 분리할 예정입니다.
...
- semantic split / merge는 OpenAI 호환 embedding client를 통해 optional하게 붙습니다. 기본 preset은 로컬 Ollama의 `bge-m3`입니다.
- semantic 실패 시 전체 chunking을 중단하지 않고 구조 청킹 결과를 그대로 저장합니다.
- dense indexing도 같은 OpenAI 호환 embedding client를 재사용해 수행합니다.
- Qdrant payload는 `text + page + section_title + asset reference` 중심의 최소 필드만 유지합니다.
