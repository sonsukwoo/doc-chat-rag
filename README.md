...
- 구현 완료: `Docling 기반 1차 파싱`, `LangGraph 기반 2차 전처리`, `figure/table crop`, `figure fan-out review`, `figure keep/drop 판정`, `table summary prepare-route-text/vlm 분기`, `문서 프로파일 추론`, `caption 연결/중복 정리`, `visual bbox 순서 보정`, `Markdown/HTML 재생성`, `stage3 구조 기반 chunking`, `부모 그룹(parent) 생성`, `chunk preview Markdown 생성`, `dense embedding 생성`, `Qdrant upsert`, `stage4 query embedding`, `dense top-k retrieval`, `retrieval eval 세트 및 채점 스크립트`
- 보수적으로 유지: `본문 텍스트는 가능한 한 원형 보존`
- 아직 분리 예정: `OCR Docker 레이어`, `image PDF 자동 라우팅`, `hybrid retrieval`, `reranking`, `chat serving layer`

즉, 현재 저장소는 **입력 PDF를 바로 Docling에 넣을 수 있다는 전제 아래**, `stage1(raw extraction) -> stage2(intelligent preprocessing) -> stage3(chunking + dense indexing) -> stage4(dense retrieval + evaluation)` 흐름을 구현하고 있습니다. OCR 라우팅과 hybrid retrieval, chat serving은 아직 메인 실행 플로우에 포함되어 있지 않습니다.
...
flowchart TD
    ...
    subgraph "Phase 4: Dense Retrieval"
        M --> N[질의 Embedding 생성]
        N --> O[Dense Top-k Retrieval\n: document filter / parent lookup]
...
| O | dense cosine 검색, 문서 필터 적용, parent lookup 결합 단계 | 구현 완료 |
...
현재는 OpenAI 호환 embedding client를 사용해, chunk 생성 직후 dense embedding을 만들고 Qdrant에 upsert 합니다. 이렇게 둔 이유는 다음과 같습니다.
...
### 4. Stage 4: Dense Retrieval + Evaluation

`backend/stage4.py`, `backend/stage4_retrieval/*`, `backend/stage4_eval.py` 가 담당합니다.

현재 stage4는 **dense retrieval baseline**에 집중합니다. 즉 hybrid나 reranking을 섞기 전에, 현재 chunk/index 구조로 검색이 얼마나 안정적으로 동작하는지 먼저 측정하는 단계입니다.

현재 stage4 흐름은 아래 순서입니다.

1. `chunks.json`, `parents.json` 로드
2. query를 같은 embedding 모델 공간으로 임베딩
3. Qdrant에서 dense cosine top-k 조회
...
- `backend/stage4_retrieval/qdrant.py`
  - dense query helper와 document filter
...
- `parents.json`: child chunk를 상위 문맥 그룹으로 묶은 parent payload
- `indexing.json`: stage3 dense indexing 결과 manifest
- `retrieval.json`: stage4 retrieval 결과 manifest `optional`
...
- `STAGE4_TOP_K`: 최종 dense retrieval 반환 개수
- `STAGE4_FETCH_K`: 최종 top-k를 자르기 전에 넓게 가져올 dense 후보 수
...
실행 후 `chunks.json`, `chunks.jsonl`, `chunks.md`, `parents.json`, `indexing.json`이 생성됩니다.
...
- Qdrant에는 dense cosine 검색만 수행합니다.
- `fetch_k`만큼 넓게 가져온 뒤 최종 `top_k`를 자릅니다.
...
- stage4 dense retrieval smoke test
...
- `stage4`는 **dense retrieval baseline과 retrieval eval 전용**입니다.
...
- 현재 `stage4`는 dense retrieval만 유지하고, hybrid / reranking은 아직 붙이지 않았습니다.
