...
- 최종적으로 `cleaned.json`, `cleaned.md`, `preview.html`, `chunks.json`, `chunks.jsonl`, `chunks.md`까지 재생성하기
...
- 구현 완료: `Docling 기반 1차 파싱`, `LangGraph 기반 2차 전처리`, `figure/table crop`, `figure fan-out review`, `figure keep/drop 판정`, `table summary prepare-route-text/vlm 분기`, `문서 프로파일 추론`, `caption 연결/중복 정리`, `visual bbox 순서 보정`, `Markdown/HTML 재생성`, `stage3 구조 기반 chunking`, `semantic split 보조`, `chunk preview Markdown 생성`
- 보수적으로 유지: `본문 텍스트는 가능한 한 원형 보존`
- 아직 분리 예정: `OCR Docker 레이어`, `image PDF 자동 라우팅`, `Vector DB 적재/검색 파이프라인`

즉, 현재 저장소는 **입력 PDF를 바로 Docling에 넣을 수 있다는 전제 아래**, `stage1(raw extraction) -> stage2(intelligent preprocessing) -> stage3(chunking)` 흐름을 구현하고 있습니다. OCR 라우팅과 Vector DB 적재는 아직 이 저장소의 메인 실행 플로우에 포함되어 있지 않으며, stage3도 현재는 **chunk 생성까지만** 담당합니다.
...
| J | `cleaned.json`을 기준으로 heading path를 유지한 text / table / figure chunk 초안을 만드는 단계 | 구현 완료 (`backend/stage3.py`) |
...
| M | 이후 embedding, vector DB 적재로 연결되는 출력 경계 | 예정 |
...
### 3. Stage 3: 구조 기반 Chunking + 선택적 Semantic Split

`backend/stage3.py`와 `backend/stage3_chunking/*`가 담당합니다.

stage3는 LangGraph가 아니라 **결정론적 pipeline**으로 구현했습니다. 이유는 현재 chunking 단계가 복잡한 분기나 fan-out 오케스트레이션보다, `cleaned.json`을 안정적으로 읽고 규칙적으로 chunk를 조립하는 작업에 가깝기 때문입니다.

현재 chunking 흐름은 아래 순서입니다.
...
├── stage3.py                  # Stage-3 chunking 엔트리포인트
├── stage3_chunking/
...
- `STAGE3_EMBEDDING_MODEL`: semantic split/merge와 이후 chunk embedding에 사용할 모델명
...
실행 후 `chunks.json`, `chunks.jsonl`, `chunks.md`가 생성됩니다.
...
- 이 단계는 아직 chunk 임베딩 벡터를 저장하지 않고, 임베딩 가능한 chunk payload만 생성합니다.
...
- `stage3`는 **구조 기반 chunking과 semantic split/merge 보조 전용**입니다.
...
- `stage1`, `stage3`도 각각 `stage1_parse/`, `stage3_chunking/` 패키지로 분리했습니다.
...
- `tests/`에는 stage3 chunk 생성과 `chunks.md` preview 렌더링에 대한 smoke test도 포함합니다.
...
- [ ] chunking 후처리 개선
  ...
  - overlap 적용 범위 세분화
- [ ] chunk embedding 생성 및 Vector DB 적재 단계 연결
