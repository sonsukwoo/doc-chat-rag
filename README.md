...
- 구현 완료: `Docling 기반 1차 파싱`, `LangGraph 기반 2차 전처리`, `figure/table crop`, `figure keep/drop 판정`, `table summary`, `visual bbox 순서 보정`, `Markdown/HTML 재생성`
...
- `document_profile`을 먼저 만들고, figure relevance 판단은 문서 전체 주제와 같은 페이지 문맥을 함께 참고합니다.
