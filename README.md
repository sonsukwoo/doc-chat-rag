...
- 구현 완료: `Docling 기반 1차 파싱`, `LangGraph 기반 2차 전처리`, `figure/table crop`, `figure keep/drop 판정`, `table summary prepare-route-text/vlm 분기`, `문서 프로파일 추론`, `caption 연결/중복 정리`, `visual bbox 순서 보정`, `Markdown/HTML 재생성`
...
| G | figure keep/drop 판정과 table `prepare -> route -> text/vlm summary` 생성을 수행하는 visual enrichment 단계 | 구현 완료 (`review_single_figure`, `prepare_table_summary_inputs`, `route_table_summaries`, `summarize_tables_text`, `summarize_tables_vlm`) |
...
9. `prepare_table_summary_inputs`: table summary용 caption/html/context/crop 경로를 정리해 상태에 적재
10. `route_table_summaries`: table별 `text` 또는 `vlm` 경로를 batch 라우팅
11. `summarize_tables_text`: text 경로 table과 image 없는 fallback table을 저가 텍스트 모델로 batch 요약
12. `summarize_tables_vlm`: image가 필요한 table만 VLM으로 batch 요약
13. `clean_elements`: caption dedupe, table-like figure 중복 제거, visual 결과 반영
14. `resolve_visual_order_outliers`: 같은 페이지의 visual outlier만 bbox 기준으로 보정
15. `render_markdown`: 최종 Markdown 생성
16. `render_preview_html`: 검수용 HTML 생성
17. `write_outputs`: cleaned 산출물 저장
...
table은 하나의 거대한 노드가 아니라 아래 단계로 나눠 처리합니다.
...
현재 구현은 LangGraph 노드를 잘게 나누되, 실제 모델 호출은 각 단계에서 `batch()`를 유지합니다. 즉 코드 구조는 명확하게 나누고, 비용은 per-table fan-out으로 늘리지 않는 쪽을 택했습니다.
