...
- 구현 완료: `Docling 기반 1차 파싱`, `LangGraph 기반 2차 전처리`, `figure/table crop`, `figure fan-out review`, `figure keep/drop 판정`, `table summary prepare-route-text/vlm 분기`, `문서 프로파일 추론`, `caption 연결/중복 정리`, `visual bbox 순서 보정`, `Markdown/HTML 재생성`, `stage3 구조 기반 chunking`, `부모 그룹(parent) 생성`, `chunk preview Markdown 생성`, `dense embedding 생성`, `Qdrant hybrid(dense+BM25) upsert`, `thread-scoped stage3 indexing payload`, `stage4 query embedding`, `dense / hybrid retrieval`, `thread-scoped retrieval filter`, `cross-encoder rerank(optional)`, `retrieval eval 세트 및 채점 스크립트`, `문서 단위 저장소(document store)`, `FastAPI 기반 업로드/실행 API`, `thread/document review 오케스트레이션`, `React/Vite 검수 + 채팅 UI`, `Postgres app schema bootstrap`, `thread_id 단일 식별자 리팩터링`, `thread CRUD / 영구 삭제`, `stage5 LangGraph chatbot graph`, `stage5 tool-calling agent loop`, `stage5 search_thread_knowledge tool wiring`, `stage5 grounded compose answer`, `profile-only / conversation-memory / retrieve-chunks 분기`, `thread chat history/checkpoint 조회`, `근거/도구/검색 경로 trace UI`
- 보수적으로 유지: `본문 텍스트는 가능한 한 원형 보존`
- 아직 분리 예정: `OCR Docker 레이어`, `image PDF 자동 라우팅`, `chat serving layer`

즉, 현재 저장소는 **입력 PDF를 바로 Docling에 넣을 수 있다는 전제 아래**, `stage1(raw extraction) -> stage2(intelligent preprocessing) -> review overlay(optional) -> stage3(chunking + indexing) -> stage4(retrieval + evaluation) -> stage5(chat graph)` 흐름까지 기반을 구현하고 있습니다. 현재 **런타임 기본값**은 `dense retrieval`이며, `hybrid retrieval`과 `reranking`은 같은 stage4 경로에서 on/off 가능한 실험 옵션으로 유지합니다. OCR 라우팅과 완전한 production chat serving은 아직 메인 실행 플로우에 포함되어 있지 않습니다.

### 현재 로컬 웹앱 진행 상태

현재는 CLI만 있는 상태가 아니라, **로컬 웹앱으로 thread 생성 -> 문서 업로드 -> stage1/stage2 실행 -> 검수 -> finalize(stage3) -> 채팅**까지 이어지는 최소 작업 흐름을 이미 붙여 둔 상태입니다.
...
- 채팅 단위: `room_id`가 아니라 `thread_id`를 최상위 식별자로 사용
- 사이드바: thread 목록 조회, 새 thread 생성, thread 삭제 지원
- 채팅 UI: 메시지별 `근거 및 진행 보기` 확장 패널에서 선택 문서, 검색 모드, 리랭크 여부, 사용 툴, 검색 청크, 로그를 확인 가능
- 시각 자료: 표/그림 근거가 잡히면 답변 말풍선 아래에 원본 visual asset을 inline으로 표시 가능
- 기본 라우트:
  - `/`: thread 목록 + 새 thread 생성 + 첫 문서 업로드
  - `/threads/:threadId/documents/:documentId/review`: stage2 검수 화면
  - `/threads/:threadId/chat`: thread 단위 채팅 화면
...
- stage별 책임은 그대로 유지하면서도, 프론트에서는 thread/document 단위 API만 호출하면 되기 때문
- stage2 자동 결과를 사람이 빠르게 보정한 뒤 stage3로 넘길 수 있기 때문
- 추후 Tauri로 감싸더라도, 내부 처리 엔진은 그대로 재사용할 수 있기 때문

### 현재 chat 준비 상태

현재 stage5는 아직 품질 튜닝 중이지만, **thread-scoped retrieval + ToolNode + grounded compose answer + chat history restore**까지는 실제로 연결된 상태입니다.

- Postgres
  - `app_chat`, `app_doc`, `app_pipeline`, `app_checkpoint` schema를 사용합니다.
  - LangGraph `PostgresSaver` checkpoint는 `app_checkpoint`에 저장됩니다.
  - 최상위 대화 단위는 `app_chat.threads`이며, 문서 연결은 `app_doc.thread_documents`로 관리합니다.
  - 기존 `room` 계열 테이블 대신 `thread_id` 기준 구조를 사용합니다.
- stage3 indexing
  - `thread_id`를 받으면 Qdrant payload와 point id에 반영합니다.
  - 컬렉션 이름은 저장값이 아니라 `thread_id -> collection_name` 규칙으로 계산합니다.
  - thread finalize-review 흐름에서만 stage3를 열어 두어, thread 범위를 벗어난 단독 stage3 실행은 막습니다.
- stage4 retrieval
  - `thread_id + active_document_ids` filter를 받는 thread-aware 검색 경로를 지원합니다.
  - artifact 기반 retrieval pipeline과 별도로, stage5에서 바로 호출할 수 있는 `search_thread_knowledge()` 서비스 함수를 둡니다.
  - 여러 문서를 동시에 다루는 경우 문서별 독립 검색을 병렬로 수행한 뒤 후보를 병합하고 전역 리랭크를 적용할 수 있습니다.
- stage5 chatbot
  - `thread_id`로 Postgres thread context를 읽고, 해당 thread의 active document 목록을 기본 검색 범위로 사용합니다.
  - 질문 분류는 `direct`, `conversation_memory`, `profile_only`, `retrieve_chunks` 중 하나로 정리합니다.
  - `profile_only`는 문서 제목/유형/핵심 토픽/짧은 요약만으로 답할 수 있는 설명·개요 질문에만 사용합니다.
  - `conversation_memory`는 "지금까지 / 방금 / 이전 대화"처럼 체크포인트에 저장된 대화 맥락을 직접 읽어 답하는 경로입니다.
  - `search_thread_knowledge` 툴은 실제 stage4 thread-scoped 검색을 호출합니다.
  - 툴은 모델 인자로 `thread_id`를 직접 받지 않고, LangGraph `runtime.state`에서 thread/document 범위를 읽습니다.
  - `retrieve_chunks`로 분류된 질문은 모델이 툴 호출을 빼먹더라도 현재 턴에서 최소 1회 검색이 강제되도록 되어 있습니다.
  - `agent_llm -> ToolNode -> grounding_check -> fallback_or_retrieve_deeper -> compose_answer_with_citations` 흐름이 실제로 연결되어 있습니다.
  - `expand_context_window`, `load_visual_asset`는 현재 Postgres runtime metadata를 읽는 실제 loader까지 연결돼 있습니다.
  - 근거가 부족할 때의 clarification은 가능한 한 **검색 후 부족한 경우**에 발생하도록 정리했습니다.
  - thread별 checkpoint를 읽어 채팅 기록과 미해결 clarification 상태를 프론트에서 복원합니다.
  - 미해결 clarification 상태에서 사용자가 짧게 문서 별칭만 답하면 backend가 후속 답변으로 해석해 resume 할 수 있습니다.
- frontend trace
  - assistant 메시지는 citation, evidence chunk, visual asset, debug trace를 함께 렌더링할 수 있습니다.
  - trace에는 선택 문서, 문서별 질의, 실행 검색 모드, 리랭크 적용 여부, 툴별 상태, 현재 턴 로그가 포함됩니다.

### 최근 stage5 운영 원칙

최근 채팅 설계는 아래 원칙으로 계속 조정하고 있습니다.

- **문서 프로파일은 우선 문서 선택과 profile-only 설명용으로 사용**합니다.
- **실제 근거 답변은 `retrieve_chunks` 경로에서 검색된 청크를 기준으로 작성**합니다.
- **일반 인사/스몰토크는 문서 검색 없이 direct 응답으로 빠집니다.**
- **이전 대화 회상 질문은 `conversation_memory`로 처리**해, profile-only와 섞이지 않게 합니다.
- **여러 문서 검색은 문서별 독립 검색 -> 병합 -> 전역 리랭크** 구조를 기본으로 가져갑니다.
- **clarification은 가능한 한 검색 후 missing evidence가 확인된 경우에만 요청**하는 방향을 유지합니다.

다만 현재 다중 문서 검색 품질은 아직 조정 중입니다. 특히 `per_document_queries`와 프로파일 앵커링을 retrieval query에 얼마나 개입시킬지는 실험 중이며, 현재 방향은 **문서 선택은 프로파일 기반으로 하되 실제 retrieval query는 가능한 한 사용자 원문을 보존**하는 쪽입니다.

### 최근 검증 상태

최근 로컬 기준으로 아래 항목을 다시 확인했습니다.

- Postgres schema fresh bootstrap 후 `app_chat.threads` / `app_doc.thread_documents` 기준 구조 확인
- backend health check 통과
- `frontend npm run build` 통과
- stage5 관련 테스트
  - `tests/test_stage5_chatbot.py`
  - `tests/test_chat_service.py`
- smalltalk는 검색 없이 direct 응답으로 빠지는 것 확인
- `retrieve_chunks` 경로는 모델이 툴 호출을 생략해도 최소 1회 검색이 수행되는 것 확인
...
5. hard split된 같은 prose run sibling에는 앞 chunk tail을 본문 앞에만 제한적으로 덧붙임
...
- `chunks.json` / `chunks.jsonl`
  ...
  - `heading_path`, `parent_id`, `caption`, `summary_text` 같은 구조/부가 메타는 별도 필드로 유지
- `chunks.md`
  - 사람이 한눈에 chunk 품질을 검수하기 위한 출력
  - `N번 청크`, 구분선, `heading_path`, `parent_id`, 본문을 함께 표시

즉 retrieval 품질을 해칠 수 있는 설명용 라벨은 preview에서만 보여주고, 실제 임베딩용 chunk text는 검색 친화적인 형태로 유지합니다.
...
현재 stage4는 **dense를 기본 운영값으로 두고**, 같은 컬렉션에서 hybrid retrieval과 cross-encoder rerank를 비교할 수 있게 정리돼 있습니다. 즉 운영은 dense-first로 두고, hybrid와 rerank는 필요할 때만 켜서 비교하는 구조입니다.
...
5. 필요 시 cross-encoder rerank로 상위 후보 relevance 재정렬
6. retrieval hit에 `parent_section_title`, page 정보, asset reference를 결합
7. `retrieval.json`은 필요할 때만 저장
8. 별도 eval 스크립트로 labeled retrieval set을 채점

현재 stage4의 역할은 아래처럼 분리돼 있습니다.

- `backend/stage4_retrieval/pipeline.py`
  - retriever -> rerank(optional) -> context expansion -> output normalize 조립
- `backend/stage4_retrieval/retriever.py`
  - Qdrant dense / hybrid 검색을 LangChain `BaseRetriever`로 감싼 adapter
- `backend/stage4_retrieval/rerank.py`
  - LangChain HuggingFace cross-encoder reranker adapter
- `backend/stage4_retrieval/context.py`
  - `child / window / parent` 문맥 확장 로직
- `backend/stage4_retrieval/postprocess.py`
  - MMR 같은 retrieval 후처리 실험 옵션
...
이 평가셋에서는 현재까지 `dense`가 `hybrid`보다 더 안정적이었습니다. 다만 sparse branch를 전체 청크에 무차별 적용하지 않고, `sparse_keep + sparse_text` 정책으로 lexical anchor가 있는 청크만 BM25에 태우는 방식으로 다시 실험한 결과, hybrid는 `hit / recall`은 개선되고 `MRR`은 아직 dense보다 약한 상태까지 올라왔습니다. 이후 cross-encoder rerank를 stage4에 추가해 다시 평가한 결과, 현재 기준 최선 경로는 `dense + rerank + window`이며, `hybrid + rerank`는 `dense + rerank`를 넘지 못했습니다.
...
- 전체 질의 기준
  - dense
    ...
    - `chunk_mrr = 0.7130`
  - dense + rerank + window
    - `chunk_hit_rate = 0.9038`
    - `chunk_mean_recall = 0.8429`
    - `chunk_mrr = 0.7268`
  - sparse-filtered hybrid `(RRF 3:1)`
    - `chunk_hit_rate = 0.9231`
    - `chunk_mean_recall = 0.8654`
    - `chunk_mrr = 0.6568`
  - hybrid + rerank + window `(RRF 3:1, current)`
    - `chunk_hit_rate = 0.9038`
    - `chunk_mean_recall = 0.8429`
    - `chunk_mrr = 0.7248`
- hybrid 친화 질의 슬라이스
  - dense
    ...
    - `chunk_mrr = 0.6368`
  - sparse-filtered hybrid `(equal weight)`
    - `chunk_hit_rate = 0.9412`
    - `chunk_mean_recall = 0.9216`
    - `chunk_mrr = 0.6261`
  - dense + rerank + window `(current subset recheck)`
    - `chunk_hit_rate = 0.8824`
    - `chunk_mean_recall = 0.8529`
    - `chunk_mrr = 0.6201`
  - hybrid + rerank + window `(current subset recheck)`
    - `chunk_hit_rate = 0.8824`
    - `chunk_mean_recall = 0.8529`
    - `chunk_mrr = 0.6172`

따라서 현재 저장소는 **런타임 기본값을 `dense + window`로 두고**, 평가상 더 좋은 품질 경로는 `dense + rerank + window`로 관리합니다. 다만 현재 cross-encoder reranker는 로컬 환경에서 메모리/지연 비용이 커서 기본값으로 올리지 않았습니다. `hybrid`는 `표/그림 번호`, `정확한 API/파라미터명`, `환경변수명`, `고유명사/식별자`, `lexical anchor가 강한 질의`에서만 조건부로 켜는 전략을 채택합니다. `MMR`은 코드에 남겨 두되 기본값은 비활성화 상태입니다.
...
- overlap은 모든 chunk에 넣지 않고, **같은 prose run이 hard split으로 쪼개진 sibling chunk에만** 앞 chunk tail을 본문 앞에 제한적으로 덧붙입니다.
...
│   └── routes/
│       ├── documents.py       # 문서 업로드 / 조회 / stage2 preview 라우트
│       ├── pipeline.py        # stage1 / stage2 / stage3 실행 라우트
│       └── review.py          # review source / decisions / apply / asset 라우트
├── common/
│   ├── __init__.py
│   └── document_ids.py        # stage 산출물 경로에서 공통 document_id 추론 helper
├── document_store/
│   ├── schemas.py             # 문서 저장소 메타 스키마
...
│   ├── sparse_policy.py       # BM25 sparse branch keep/drop 정책
...
├── stage4.py                  # Stage-4 retrieval 엔트리포인트
├── stage4_eval.py             # Stage-4 retrieval evaluation CLI
├── stage4_retrieval/
│   ├── config.py              # retrieval runtime 설정
│   ├── context.py             # child/window/parent 문맥 확장
│   ├── evaluation.py          # retrieval eval 집계 로직
│   ├── parents.py             # parent lookup 로더
│   ├── pipeline.py            # retriever + 후처리 + output normalize
│   ├── postprocess.py         # MMR 등 retrieval 후처리 실험 옵션
│   ├── qdrant.py              # dense / hybrid retrieval helper
│   ├── rerank.py              # cross-encoder rerank helper
│   ├── retriever.py           # LangChain BaseRetriever adapter
│   └── schemas.py             # retrieval 입출력 스키마
└── outputs/                   # 문서별 산출물 저장소

tests/
├── test_document_ids.py       # 공통 document_id 추론 규칙 test
...
- `metadata`
  ...
  - `sparse_keep`
  - `sparse_text`
  - `sparse_exclude_reason`
  - `caption`
  - `summary_text`

여기서 중요한 점은, `heading_path`, `parent_id`, `sparse_*` 메타는 구조/검색 보조 신호로 남기되, 실제 `text` 안에는 `섹션:` 같은 설명용 라벨을 넣지 않는다는 것입니다. split된 prose chunk의 overlap은 별도 `overlap_text` 필드가 아니라 최종 `text` 앞부분에만 제한적으로 반영합니다.
...
- `STAGE4_ENABLE_RERANK`: stage4에서 cross-encoder rerank를 켤지 여부
- `STAGE4_RERANK_MODEL`: rerank에 사용할 Hugging Face cross-encoder 모델명
- `STAGE4_RERANK_DEVICE`: rerank 실행 장치. 예: `cpu`, `mps`
...
현재 FastAPI는 여전히 low-level document API를 유지하지만, **실제 웹앱 메인 흐름은 thread 중심 API**를 사용합니다.

- thread
  - `GET /threads`
  - `POST /threads`
  - `POST /threads/with-document`
  - `POST /threads/bootstrap`
  - `GET /threads/{thread_id}`
  - `PATCH /threads/{thread_id}`
  - `DELETE /threads/{thread_id}`
- thread document flow
...
- low-level document/review API
  - `POST /documents/upload`
  - `GET /documents`
  - `GET /documents/{document_id}`
  - `POST /documents/{document_id}/stage1`
  - `POST /documents/{document_id}/stage2`
  - `POST /documents/{document_id}/stage3` (`finalize-review` 경로만 허용하고 단독 stage3는 400으로 막습니다.)
...
즉, 프론트는 stage 내부 구현을 직접 알 필요 없이 **thread 생성 -> 문서 업로드 -> review -> finalize -> chat** 흐름만 호출하면 됩니다. low-level document API는 디버그, 수동 stage 실행, review asset 제공 용도로 남겨 둔 상태입니다.
...
- split된 prose chunk는 앞 chunk tail이 현재 chunk 본문 앞에만 제한적으로 반영됩니다.
...
- `rerank`가 켜져 있으면 dense/hybrid retrieval 결과에 cross-encoder rerank를 한 번 더 적용합니다.
- 현재 런타임 기본값은 `dense`이며, hybrid와 rerank는 비교/승격 실험용 옵션으로 유지합니다.
...
- 현재 `stage4`는 dense를 기본으로 두고, hybrid retrieval과 cross-encoder rerank도 옵션으로 함께 지원합니다.
...
- [ ] stage4 retrieval 고도화
  - query 유형별 dense / hybrid / rerank 조건부 라우팅
  - answer eval 기준 retrieval 경로 재평가
  - rerank용 더 가벼운 운영 모델 검토
