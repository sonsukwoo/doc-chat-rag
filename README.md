...
이 저장소는 문서를 먼저 `element list`로 분해한 뒤, LangGraph 기반 2차 전처리에서 문서 맥락을 보고 노이즈 제거, 짧은 텍스트 semantic gate, visual crop, figure relevance 판정, table summary 생성을 수행하는 방식을 택합니다.
...
- 구현 완료: `Docling 기반 1차 파싱`, `LangGraph 기반 2차 전처리`, `figure/table crop`, `figure keep/drop 판정`, `table summary text/vlm 라우팅`, `문서 프로파일 추론`, `짧은 텍스트 semantic keep/drop gate`, `visual bbox 순서 보정`, `Markdown/HTML 재생성`
- 부분 구현: `웹 문서용 레이아웃 조각 복원`
...
| D | caption 연결, 텍스트 normalize, heading 승격, `document_profile` 생성 단계 | 구현 완료 |
| E | 헤더/푸터, 페이지 카운터, 명백한 junk figure와 generic `full_page_image`를 제거하는 단계 | 구현 완료 (`rule_filter_elements`) |
| E-1 | 짧은 paragraph/list 후보를 exact duplicate 기준으로 batch 검토해 text junk를 제거하거나 인접 본문에 붙이는 단계 | 구현 완료 (`review_short_text_candidates`) |
...
5. `rule_filter_elements`: 헤더/푸터, 페이지 번호, 명백한 junk figure와 generic `full_page_image` 제거
6. `review_short_text_candidates`: 짧은 text 후보를 규칙 + batch LLM으로 keep/drop/attach 처리
7. `build_visual_tasks`: crop/VLM 대상인 figure, table 작업 목록 생성
8. `crop_visuals`: bbox 기준으로 표/그림 이미지 crop 저장
9. `review_single_figure`: figure별 keep/drop 및 summary 생성
10. `summarize_tables`: table별 `text/vlm` 라우팅 후 핵심 summary 생성
11. `clean_elements`: caption dedupe, table-like figure 중복 제거, visual 결과 반영
12. `resolve_visual_order_outliers`: 같은 페이지의 visual outlier만 bbox 기준으로 보정
13. `render_markdown`: 최종 Markdown 생성
14. `render_preview_html`: 검수용 HTML 생성
15. `write_outputs`: cleaned 산출물 저장
...
즉 `document_profile`은 "이 문서는 대체 무엇에 대한 문서인가?"를 먼저 압축해두고, 이후 figure/table/짧은 텍스트 판단에서 그 문맥을 계속 재사용하기 위한 장치입니다. 실제 프롬프트에는 pretty JSON 전체를 반복해서 넣지 않고, 제목 / 문서 유형 / 핵심 토픽 / 관련 visual 힌트만 **짧은 문자열로 압축한 형태**로 재사용합니다.
...
### 4. 짧은 텍스트 정크는 어떻게 처리하는가?

웹 PDF나 e북 계열 문서에서는 `paragraph`, `list`, `code`가 지나치게 잘게 쪼개지며, 광고 문구나 UI 조각이 본문처럼 섞여 들어올 수 있습니다. 이를 줄이기 위해 `review_short_text_candidates` 노드가 `rule_filter_elements` 뒤에서 먼저 동작합니다.
...
- 규칙으로 확실한 정크 제거
  ...
  - 순수 기호/구두점 조각
  - 명백한 한 글자 정크
- 그 다음 `paragraph`, `list`, `code` 중 `60자 이하` 짧은 요소만 후보로 수집
- 공백/구두점을 정리한 `normalized_text` 기준으로 **완전히 같은 텍스트끼리만** 그룹핑
- 각 그룹 대표 1개만 저가 텍스트 모델에 `batch()`로 보냄
- 1차 결과는 `keep`, `drop`, `needs_context`
- `needs_context`만 다시 앞뒤 본문 텍스트를 붙여 2차 판정
- 2차 결과는 `keep`, `drop`, `attach_to_prev`, `attach_to_next`

즉 이 단계는 짧은 텍스트를 무조건 지우는 것이 아니라, **반복 잡음은 줄이고, 애매한 짧은 표지어나 본문 조각은 앞뒤 문맥을 보고 인접 본문에 붙이거나 유지하는 방식**으로 동작합니다.
...
초기 버전은 figure/table/문서 프로파일/짧은 텍스트 검토가 모두 같은 VLM 경로를 타기 쉬워 토큰 사용량이 커질 수 있었습니다. 현재는 아래 방식으로 비용을 줄이는 방향으로 정리했습니다.

- 텍스트 작업과 이미지 작업의 모델을 분리
  - `OPENAI_VLM_MODEL`: figure VLM 판단, VLM table summary
  - `OPENAI_TEXT_MODEL`: document profile, 짧은 텍스트 정리, table text summary
...
- 짧은 텍스트는 exact duplicate 대표만 batch 판정하고 결과를 그룹 전체에 재사용
...
`cleaned.json`은 단순 디버그 덤프가 아니라, 이후 chunking / embedding으로 넘기기 쉬운 **후처리 기준 intermediate**로 유지하는 것을 목표로 합니다.
...
- 구조화 출력은 필요한 필드만 남기도록 단순화했습니다.
  ...
  - short text review: `action`
  - short text context review: `action`
