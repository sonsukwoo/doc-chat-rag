...
이 평가셋에서는 현재까지 `dense`가 `hybrid`보다 더 안정적이었습니다. 다만 sparse branch를 전체 청크에 무차별 적용하지 않고, `sparse_keep + sparse_text` 정책으로 lexical anchor가 있는 청크만 BM25에 태우는 방식으로 다시 실험한 결과, hybrid는 `hit / recall`은 개선되고 `MRR`은 아직 dense보다 약한 상태까지 올라왔습니다.
...
따라서 현재 저장소는 **검색 기본값을 `dense + window`로 두고**, hybrid는 `표/그림 번호`, `정확한 API/파라미터명`, `환경변수명`, `고유명사/식별자`, `lexical anchor가 강한 질의`에서만 조건부로 켜는 전략을 채택합니다. `MMR`은 코드에 남겨 두되 기본값은 비활성화 상태입니다.
