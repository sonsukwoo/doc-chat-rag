...
- 구현 완료: `Docling 기반 1차 파싱`, `LangGraph 기반 2차 전처리`, `figure/table crop`, `figure keep/drop 판정`, `table summary`, `문서 프로파일 추론`, `visual bbox 순서 보정`, `Markdown/HTML 재생성`
- 부분 구현: `텍스트 semantic keep/drop gate`
...
| G | figure keep/drop 판정과 table/figure summary 생성을 수행하는 visual enrichment 단계 | 구현 완료 (`review_single_figure`, `summarize_tables`) |
...
- table은 `document_profile + caption + image + optional local body context`를 바탕으로 summary를 생성합니다.
- `document_profile`을 먼저 만들고, figure relevance 판단은 문서 전체 주제, caption, 이미지, 주변 본문 텍스트를 함께 참고합니다.
